...
( set -o pipefail && xcodebuild -workspace MBGeolocation.xcworkspace -configuration Debug -scheme "MBGeolocation" -destination "$DESTINATION" -destination-timeout 300 $BUILD_ACTION 2>&1 | tee "MBGeolocation-$PLATFORM-$OPERATION.log" | xcpretty )
